...
1. Kynntu þér allt um [MQTT](https://mqtt.org) og [mqtt_as](https://github.com/peterhinch/micropython-mqtt) safnið.
1. Settu mqtt_as safnið inn á ESP_ana þína með því að keyra eftirfarandi kóða á þeim:
     ```python
     def do_connect():
         import network
     ...
     ```
1. Sýnidæmi (sendir Hallo ásamt teljara):
     <details>
...
1. Þegar þú hefur fengið MQTT til að virka áttu að gera eftirfarandi:
...
1. Sendu til baka gráðustöðuna í JSON frá fígúru á 1 sek. fresti (nota sama JSON og litastöðu).
