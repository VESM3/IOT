...
> [!Note]
> Það verður að hafa while True lykkju í main með tíma `await asyncio.sleep_ms(0)` þó hún geri ekki neitt, þetta er til að task fari af stað og keyri endurkvæmt. Það er líka hægt að hafa fastan tíma á main fallinu.
...
Búðu til stutta senu þar sem fígúran hreyfir höfuð, blikkar augun og hlær, allt á sama tíma (async). Fáðu kjálkahreyfingu í takt við hlátur. Notaðu DAC pinnann á DFPlayer-num, tengdan inn á ACD pinna 14 á ESP til nota hljóðið/hljóðbylgjurnar til að stjórna munninum.
...
klemmufall:
```python
def klemma(gildi, min_gildi, max_gildi):
  return max(min_gildi, min(gildi, max_gildi)
```
...
- Skilaðu öllum kóða á Innu
  - Passaðu að þú getir útskýrt kóðann sem þú skilar fyrir kennara. Ef nemandi notar gervigreind við að leysa verkefnið er gefið 0 (núll) fyrir verkefnið í heild.
