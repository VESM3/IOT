...
1. Myndgreining með Computer Vision (fartölva)
    ...
    1. Finndu "Computer Vision" og veldu þar "Create" og settu inn upplýsingarnar eins og hér fyrir neðan:
    ![búa til CV](https://raw.githubusercontent.com/VESM3/IOT/refs/heads/main/Myndir/azure-cv-create.png)
    Skrunaðu svo neðst á síðuna og hakaðu í svæðið þar sem þú samþykkir allt sem talið er upp og smelltu svo á "Review + create" og síðan á "Create". Á síðunni sem næst kemur upp smellir þú svo á "Go to resource". Neðarlega á síðunni sem þá kemur upp finnur þú svo tvo API lyklana (þú þarft bara að nota annan, þú ræður hvorn) og "Endpoint" sem þú þarft að nota.
    1. Fylgdu eftir [Analyse image](https://learn.microsoft.com/en-us/azure/ai-services/computer-vision/quickstarts-sdk/image-analysis-client-library-40?tabs=visual-studio%2Clinux&pivots=programming-language-python#analyze-image) sýnidæminu frá Microsoft á fartölvunni þinni til að sjá að þetta virki allt hjá þér. Taktu út `try/except` blokkina og settu þetta í staðinn: 
        ```python
        key = API_LYKILLINN_ÞINN
        endpoint = ENDPOINTIÐ_ÞITT
        ```
   <!--1. Gerðu viðeigandi stillingar hjá Azure [myndband](https://www.youtube.com/watch?v=1VB_QrHm_nY&ab_channel=JieJenn) til að geta notað Computer Vision. Hér eru frekari [leiðbeiningar](https://www.pluralsight.com/guides/computer-vision-with-microsoft-azure).-->
    1. Skoðaðu [Azure SDK for Python](https://learn.microsoft.com/en-us/python/api/overview/azure/ai-vision-imageanalysis-readme?view=azure-python-preview), ýmis kóðadæmi [hér](https://github.com/Azure/azure-sdk-for-python/tree/azure-ai-vision-imageanalysis_1.0.0b3/sdk/vision/azure-ai-vision-imageanalysis/samples) og sýndu svo hæfni þína til að nota það. Sýndu kennara tvo mismunandi hluti sem þú getur gert með Azure Computer Vision. Frjáls aðferð.
1. Notaðu RPi, PiCamera2 (local ljósmynd) og Computer Vision saman til að gera svipað og þú gerðir hér fyrir ofan en sýndu tvö atriði (ekki sama og þú gerðir hér fyrir ofan). ATH. þegar þú notar setur inn Python Image Analysis þá þarftu að gera eftirfarandi: `pip install azure-ai-vision-imageanalysis --break-system-packages`.

<!--Sjá [kóðabúta](https://github.com/Azure-Samples/cognitive-services-quickstart-code/blob/master/python/ComputerVision/ImageAnalysisQuickstart.py) úr skránni. (ekki nota `pillow` safnið) -->
